...
DROP POLICY IF EXISTS "Users can view own palette" ON user_palette;
CREATE POLICY "Users can view own palette"
  ON user_palette
  FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own palette" ON user_palette;
CREATE POLICY "Users can insert own palette"
  ON user_palette
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own palette" ON user_palette;
CREATE POLICY "Users can update own palette"
  ON user_palette
  FOR UPDATE
  USING (auth.uid() = user_id);
