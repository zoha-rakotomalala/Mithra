...
function convertToPainting(db: any): Painting {
  ...
  if (
    ...
    m.image_id &&
    db.legacy_id &&
    db.legacy_id.indexOf('chicago-') === 0
  ) {
    imageUrl =
...
export async function getCachedPaintings(
  ...
  try {
    ...
    const { data: entry, error: entryError } = await supabase
      ...
      .eq('museum_id', museumLegacyId)
      .single();
    if (entryError && entryError.code !== 'PGRST116') throw entryError;
    if (entry && entry.painting_ids.length > 0) {
      const { data: paintings, error: pe } = await supabase
        .from('paintings')
        .select('*')
        .in('id', entry.painting_ids);
      if (pe) throw pe;
...
export async function updateCacheWithFreshResults(
  ...
  try {
    ...
    const stats = {
      ...
      updated: 0,
      unchanged: 0,
      legacyToUuid: {} as Record<string, string>,
    };
    ...
    const dbPaintings = freshPaintings.map((painting: any) => {
      ...
      if (legacyId.indexOf('chicago-') === 0 && painting.image_id)
        metadata.image_id = painting.image_id;
      return {
    ...
    for (let i = 0; i < uniqueDbPaintings.length; i += batchSize) {
      const { error } = await supabase
        .from('paintings')
        .upsert(uniqueDbPaintings.slice(i, i + batchSize), {
          onConflict: 'museum_id,external_id',
          ignoreDuplicates: false,
        });
      if (error) console.error('Upsert error:', error);
    }
    const externalIds = freshPaintings.map((p: any) =>
      extractExternalId(String(p.id)),
    );
    const { data: upserted } = await supabase
      .from('paintings')
      .select('id, external_id')
      .eq('museum_id', museumUuid)
    ...
    stats.added = freshPaintings.filter(
      (p: any) => !currentIds[String(p.id)],
    ).length;
    stats.updated = freshPaintings.filter(
      (p: any) => !!currentIds[String(p.id)],
    ).length;
...
export async function getCacheFreshness(
  ...
  try {
    ...
    const ageMinutes = Math.floor(
      (Date.now() - new Date(data.last_verified_at).getTime()) / 60000,
    );
...
export async function markStaleCacheEntries(
  daysOld: number = 30,
): Promise<number> {
  try {
...
export async function cleanupOrphanedPaintings(
  daysOld: number = 180,
): Promise<number> {
  try {
    const cutoff = new Date();
    ...
    const { data: old } = await supabase
      .from('paintings')
      .select('id')
      .lt('cached_at', cutoff.toISOString());
    ...
    (caches || []).forEach((c: any) =>
      (c.painting_ids || []).forEach((id: string) => {
        referenced[id] = true;
      }),
    );
    const orphaned = old
      .map((p: any) => p.id)
      .filter((id: string) => !referenced[id]);
...
export async function getCachedPaintingById(
  paintingUuid: string,
...
export async function getCacheStats(): Promise<{
  ...
  try {
    const [
      ...
      supabase.from('paintings').select('museum_id'),
      supabase.from('search_cache').select('museum_id'),
    ]);
    const paintingsByMuseum: Record<string, number> = {};
    (mp || []).forEach((p: any) => {
      paintingsByMuseum[p.museum_id] =
        (paintingsByMuseum[p.museum_id] || 0) + 1;
    });
