...
export function CollectionProvider({ children, storage, onLoaded }: CollectionProviderProps) {
  const { user } = useAuth();
    ...
    setPaintings(previous => {
      ...
      if (user?.id) {
        ...
        if (updatedPainting) {
          ...
          syncService.upsertCollectionEntry(user.id, entry).catch(err => {
            reportSyncError(err?.message || 'Failed to sync want-to-visit status');
          });
        }
      }
  ...
  const value = useMemo(() => ({
    ...
    _refreshFromStorage,
  }), [paintings, addToCollection, removeFromCollection, isInCollection, toggleSeen, toggleWantToVisit, getPaintingsByArtist, getPaintingsByMuseum, _refreshFromStorage]);
