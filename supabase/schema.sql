...
(
    ...
    museum_id           UUID                           NOT NULL
        CONSTRAINT fk_paintings_museum REFERENCES public.museums ON DELETE RESTRICT,
...
(
    ...
    user_id    UUID,
    museum_id  UUID
        CONSTRAINT fk_visits_museum REFERENCES public.museums ON DELETE RESTRICT,
    visit_date DATE,
    notes      TEXT,
...
(
    ...
    user_id      UUID,
    painting_1id UUID NOT NULL
        CONSTRAINT fk_likes_painting REFERENCES public.paintings ON DELETE CASCADE,
    visit_id     UUID
        CONSTRAINT fk_likes_visit REFERENCES public.visits ON DELETE SET NULL,
...
(
    ...
    CONSTRAINT chk_user_palette_max_size CHECK (array_length(painting_ids, 1) IS NULL OR
...
CREATE
POLICY "Users can view own collection" ON public.user_collection
  FOR
SELECT USING (auth.uid() = user_id);
CREATE
POLICY "Users can insert own collection" ON public.user_collection
  FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE
POLICY "Users can update own collection" ON public.user_collection
  FOR
UPDATE USING (auth.uid() = user_id);
CREATE
POLICY "Users can delete own collection" ON public.user_collection
  FOR DELETE
USING (auth.uid() = user_id);

-- user_palette policies
CREATE
POLICY "Users can view own palette" ON public.user_palette
  FOR
SELECT USING (auth.uid() = user_id);
CREATE
POLICY "Users can insert own palette" ON public.user_palette
  FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE
POLICY "Users can update own palette" ON public.user_palette
  FOR
UPDATE USING (auth.uid() = user_id);
