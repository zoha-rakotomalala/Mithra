...
SELECT id, museum_id, title
FROM paintings LIMIT 10;

-- CHECK-2: See what like IDs look like
SELECT painting_id, visit_id
FROM user_painting_likes LIMIT 10;
...
VALUES ('Metropolitan Museum of Art', 'MET', 'New York City, USA',
       ...
       ('Paris Musées', 'Paris', 'Paris, France',
        ...
        '{"color":"#DB2777","country":"France","tier":3,"legacyId":"PARIS"}', true) ON CONFLICT DO NOTHING;
...
DROP
CONSTRAINT IF EXISTS user_painting_likes_painting_id_fkey;
ALTER TABLE user_painting_likes
DROP
CONSTRAINT IF EXISTS fk_likes_painting;
...
UPDATE paintings p
SET new_museum_id = m.id FROM museums m
WHERE m.metadata->>'legacyId' = p.museum_id;

-- Check how many resolved (should be close to total row count)
SELECT COUNT(*) FILTER (WHERE new_museum_id IS NOT NULL) AS resolved, COUNT(*) FILTER (WHERE new_museum_id IS NULL)     AS unresolved, COUNT(*) AS total
FROM paintings;
...
SELECT id, new_id
FROM paintings ON CONFLICT (old_id) DO NOTHING;

-- Verify mapping count matches paintings count
SELECT COUNT(*)
FROM _painting_id_migration_map;
SELECT COUNT(*)
FROM paintings;
...
SELECT id, museum_id, external_id, title
FROM paintings LIMIT 5;
...
ALTER TABLE visits
    ADD COLUMN IF NOT EXISTS museum_id_new UUID;
...
SELECT COUNT(*) FILTER (WHERE m.new_id IS NOT NULL) AS will_survive, COUNT(*) FILTER (WHERE m.new_id IS NULL)     AS will_be_deleted, COUNT(*) AS total
...
SET painting_id_new = m.new_id FROM _painting_id_migration_map m
WHERE m.old_id = upl.painting_id;

-- Delete orphaned likes (painting not found in paintings table)
DELETE
FROM user_painting_likes
WHERE painting_id_new IS NULL;
...
ALTER TABLE user_painting_likes
    ALTER COLUMN painting_id SET NOT NULL;
...
ALTER TABLE user_painting_likes
ALTER
COLUMN visit_id TYPE UUID USING visit_id::UUID;
END IF;
END $$;
...
FROM user_painting_likes;
SELECT painting_id
FROM user_painting_likes LIMIT 5;
...
(
    ...
    palette_id UUID NOT NULL REFERENCES visit_palettes
(
    id
) ON DELETE CASCADE,
...
DROP
POLICY IF EXISTS museums_select ON museums;
CREATE
POLICY museums_select ON museums FOR
SELECT TO authenticated USING (true);

-- Paintings
ALTER TABLE paintings ENABLE ROW LEVEL SECURITY;
DROP
POLICY IF EXISTS paintings_select ON paintings;
DROP
POLICY IF EXISTS paintings_insert ON paintings;
DROP
POLICY IF EXISTS paintings_update ON paintings;
CREATE
POLICY paintings_select ON paintings FOR
SELECT TO authenticated USING (true);
CREATE
POLICY paintings_insert ON paintings FOR INSERT TO authenticated WITH CHECK (true);
CREATE
POLICY paintings_update ON paintings FOR
UPDATE TO authenticated USING (true);

-- Visits
ALTER TABLE visits ENABLE ROW LEVEL SECURITY;
DROP
POLICY IF EXISTS visits_select ON visits;
DROP
POLICY IF EXISTS visits_insert ON visits;
DROP
POLICY IF EXISTS visits_update ON visits;
DROP
POLICY IF EXISTS visits_delete ON visits;
CREATE
POLICY visits_select ON visits FOR
SELECT TO authenticated USING (auth.uid() = user_id);
CREATE
POLICY visits_insert ON visits FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE
POLICY visits_update ON visits FOR
UPDATE TO authenticated USING (auth.uid() = user_id);
CREATE
POLICY visits_delete ON visits FOR DELETE
TO authenticated USING (auth.uid() = user_id);

-- user_painting_likes
ALTER TABLE user_painting_likes ENABLE ROW LEVEL SECURITY;
DROP
POLICY IF EXISTS likes_select ON user_painting_likes;
DROP
POLICY IF EXISTS likes_insert ON user_painting_likes;
DROP
POLICY IF EXISTS likes_delete ON user_painting_likes;
CREATE
POLICY likes_select ON user_painting_likes FOR
SELECT TO authenticated USING (auth.uid() = user_id);
CREATE
POLICY likes_insert ON user_painting_likes FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE
POLICY likes_delete ON user_painting_likes FOR DELETE
TO authenticated USING (auth.uid() = user_id);

-- visit_palettes
ALTER TABLE visit_palettes ENABLE ROW LEVEL SECURITY;
DROP
POLICY IF EXISTS palettes_select ON visit_palettes;
DROP
POLICY IF EXISTS palettes_insert ON visit_palettes;
DROP
POLICY IF EXISTS palettes_update ON visit_palettes;
DROP
POLICY IF EXISTS palettes_delete ON visit_palettes;
CREATE
POLICY palettes_select ON visit_palettes FOR
SELECT TO authenticated
    USING (EXISTS (SELECT 1 FROM visits v WHERE v.id = visit_palettes.visit_id AND v.user_id = auth.uid()));
CREATE
POLICY palettes_insert ON visit_palettes FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM visits v WHERE v.id = visit_palettes.visit_id AND v.user_id = auth.uid()));
CREATE
POLICY palettes_update ON visit_palettes FOR
UPDATE TO authenticated
    USING (EXISTS (SELECT 1 FROM visits v WHERE v.id = visit_palettes.visit_id AND v.user_id = auth.uid()));
CREATE
POLICY palettes_delete ON visit_palettes FOR DELETE
TO authenticated
  USING (EXISTS (SELECT 1 FROM visits v WHERE v.id = visit_palettes.visit_id AND v.user_id = auth.uid()));
...
DROP
POLICY IF EXISTS palette_paintings_select ON visit_palette_paintings;
DROP
POLICY IF EXISTS palette_paintings_insert ON visit_palette_paintings;
DROP
POLICY IF EXISTS palette_paintings_delete ON visit_palette_paintings;
CREATE
POLICY palette_paintings_select ON visit_palette_paintings FOR
SELECT TO authenticated
...
CREATE
POLICY palette_paintings_insert ON visit_palette_paintings FOR INSERT TO authenticated
...
CREATE
POLICY palette_paintings_delete ON visit_palette_paintings FOR DELETE
TO authenticated
  USING (EXISTS (SELECT 1 FROM visit_palettes vp JOIN visits v ON v.id = vp.visit_id WHERE vp.id = visit_palette_paintings.palette_id AND v.user_id = auth.uid()));

-- Final verification: list all tables and their row counts
SELECT 'museums' AS tbl, COUNT(*)
FROM museums
UNION ALL
SELECT 'paintings', COUNT(*)
FROM paintings
UNION ALL
SELECT 'user_painting_likes', COUNT(*)
FROM user_painting_likes
UNION ALL
SELECT 'visits', COUNT(*)
FROM visits
UNION ALL
SELECT 'visit_palettes', COUNT(*)
FROM visit_palettes
UNION ALL
SELECT 'visit_palette_paintings', COUNT(*)
